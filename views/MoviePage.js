...
document.querySelector('.my-list-button').addEventListener('click', function () {
    // Retrieve movie details from the page
    const title = document.querySelector('.movie-title').innerText;
    const description = document.querySelector('.banner .content p').innerText;
    const imageUrl = document.querySelector('.banner').style.backgroundImage.replace(/url\("|"\)/g, '');

    // Retrieve movieId from localStorage
    const movieId = localStorage.getItem('selectedMovieId');
    ...
    fetch('/watchlist/add', {
        ...
        body: JSON.stringify({ movieId, title, description, imageUrl })
    })
        .then(response => {
            if (response.ok) {
...
document.querySelector('.rate-button').addEventListener('click', function(event) {
    ...
    // Make an AJAX request to add the movie rating
    fetch('/myratings/add', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        ...
        body: JSON.stringify({ movieId, title, description, imageUrl, rating: ratingValue })
    })
    .then(response => {
        if (response.ok) {
